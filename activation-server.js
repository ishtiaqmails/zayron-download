...
const server = http.createServer(async (req, res) => {
  ...
  if (p.indexOf('/admin/act') >= 0 && req.method === 'POST') {
    ...
    try {
      ...
      if (a === 'forceLatest') { if (!adminOnly()) return json(res, 403, { error: 'admins only' }); const ap = b.app; const latest = db.maxver[ap] || 0; if (!latest) return json(res, 200, { ok: false, error: 'no live version seen yet for this app' }); if (!db.config.update[ap]) db.config.update[ap] = {}; const cu = db.config.update[ap];
        // direct auto-install URLs (where the CI publishes) — so 1-click forces AND auto-installs, no typing
        const DIRECT = { android: 'https://github.com/ishtiaqmails/zayron-download/releases/latest/download/Zayron.apk', windows: 'https://github.com/ishtiaqmails/zayron-download/releases/download/windows/Zayron-Setup.exe' };
        cu.on = true; cu.min = latest; cu.url = DIRECT[ap] || String(b.url || cu.url || '').trim(); if (!cu.msg) cu.msg = 'A new version is available. Please update to continue.'; save(db); writeVersionJson(); return json(res, 200, { ok: true, min: latest }); }
